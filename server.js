...
const NotificationSchema = new mongoose.Schema({
    message: String,
    type: { type: String, enum: ['subscription', 'system', 'warning', 'success'], default: 'system' },
...
const ContentSchema = new mongoose.Schema({
    ...
    downloadedBy: [DownloadRecordSchema],
    parts: [PartSchema],
...
const authMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (user.subscription.status === 'active' && user.subscription.expiresAt && new Date() > new Date(user.subscription.expiresAt)) {
            ...
            user.subscription.duration = 'none';
        ...
        if (existingDevice) {
            ...
            if (user.role !== 'admin' && user.subscription.plan !== 'free' && user.devices.length >= user.subscription.maxDevices) {
                user.isFlagged = true;
                user.flagReason = 'Device limit exceeded (' + user.subscription.maxDevices + ' devices). Contact admin.';
                await user.save();
...
const adminMiddleware = (req, res, next) => {
    if (!req.user || (req.user.role !== 'admin' && req.user.role !== 'content_creator')) {
...
const headAdminMiddleware = (req, res, next) => {
    if (!req.user || req.user.adminLevel !== 'head') {
        return res.status(403).json({ error: 'Only Head Admin can perform this action.' });
    }
    next();
};

const subAdminOrAbove = (req, res, next) => {
    if (!req.user || (req.user.adminLevel !== 'head' && req.user.adminLevel !== 'sub')) {
...
function canStream(url) {
    if (!url) return false;
    return url.includes('pixeldrain.com') || url.includes('youtube.com') || url.includes('vimeo.com') || url.includes('mega.nz');
}
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        res.status(201).json({ token, user: { id: user._id, email, role: user.role, fullName: user.fullName, subscription: user.subscription, isEmailVerified: true }, message: 'Account created!' });
    } catch (err) {
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (existingDevice) {
            ...
            if (user.role !== 'admin' && user.subscription.plan !== 'free' && user.devices.length >= user.subscription.maxDevices) {
                user.isFlagged = true;
                user.flagReason = 'Device limit exceeded (' + user.subscription.maxDevices + ' devices).';
                await user.save();
        ...
        res.json({ token, user: { id: user._id, email: user.email, role: user.role, adminLevel: user.adminLevel, fullName: user.fullName, subscription: user.subscription, deviceCount: user.deviceCount, isFlagged: user.isFlagged }, expiringSoon });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.get('/api/announcements', async (req, res) => {
    res.json(await Announcement.find({ isActive: true }).sort({ createdAt: -1 }).limit(1));
});
...
app.put('/api/admin/announcements/:id', authMiddleware, subAdminOrAbove, async (req, res) => {
    await Announcement.findByIdAndUpdate(req.params.id, req.body);
    res.json({ success: true });
});

app.delete('/api/admin/announcements/:id', authMiddleware, headAdminMiddleware, async (req, res) => {
...
app.post('/api/contents/:id/download', async (req, res) => {
    try {
        ...
        if (content.type === 'movie' && content.parts && content.parts.length > partIndex) {
            videoUrl = content.parts[partIndex].videoUrl;
            itemAccessLevel = content.parts[partIndex].accessLevel || 'free';
        } else if (content.type === 'series' && content.seasons && content.seasons[seasonIndex] && content.seasons[seasonIndex].episodes && content.seasons[seasonIndex].episodes[episodeIndex]) {
            videoUrl = content.seasons[seasonIndex].episodes[episodeIndex].videoUrl;
            itemAccessLevel = content.seasons[seasonIndex].episodes[episodeIndex].accessLevel || 'free';
...
app.post('/api/comments/:contentId', async (req, res) => {
    ...
    const content = await Content.findById(req.params.contentId);
    if (!content) return res.status(404).json({ error: 'Not found' });
    content.comments.push({ userName, text });
    await content.save();
...
app.post('/api/comments/:contentId/:commentId/like', async (req, res) => {
    try {
        ...
        if (userId) {
            alreadyLiked = comment.likedBy.some(id => id.toString() === userId.toString());
        } else {
            alreadyLiked = comment.likedByDevice.includes(deviceId);
        }
...
app.get('/api/plans', (req, res) => {
    res.json({
        ...
        basic: { name: 'Basic', weekly: 300, monthly: 500, quarterly: 1200, yearly: 3000, features: ['No ads', '720p', 'Mobile download'] },
        standard: { name: 'Standard', weekly: 500, monthly: 1000, quarterly: 2500, yearly: 7000, features: ['No ads', '1080p', 'Offline download', '2 devices'] },
        premium: { name: 'Premium', weekly: 1000, monthly: 2000, quarterly: 5000, yearly: 15000, features: ['No ads', '2K', '5 devices', '4K support'] },
        ultimate: { name: 'Ultimate', weekly: 1500, monthly: 3000, quarterly: 7500, yearly: 20000, features: ['Everything', '4K', 'Unlimited devices'] }
    });
});
...
app.post('/api/subscribe', authMiddleware, upload.single('paymentScreenshot'), async (req, res) => {
    try {
        ...
        const plans = {
            basic: { weekly: 300, monthly: 500, quarterly: 1200, yearly: 3000 },
            standard: { weekly: 500, monthly: 1000, quarterly: 2500, yearly: 7000 },
            premium: { weekly: 1000, monthly: 2000, quarterly: 5000, yearly: 15000 },
            ultimate: { weekly: 1500, monthly: 3000, quarterly: 7500, yearly: 20000 }
        };
...
app.get('/api/admin/me', authMiddleware, adminMiddleware, async (req, res) => {
    const user = await User.findById(req.user.id).select('-password');
    res.json({ ...user.toObject(), isHeadAdmin: user.adminLevel === 'head' });
});
...
app.post('/api/admin/upload', authMiddleware, adminMiddleware, upload.fields([{ name: 'thumbnail', maxCount: 1 }, { name: 'trailer', maxCount: 1 }, { name: 'video', maxCount: 1 }]), async (req, res) => {
    try {
        const { type, title, description, category, year, director, cast, translator, language, country, accessLevel, quality, ageRating, tags, isFeatured, isTrending, videoSource, externalLink, partAccessLevel, seasonNumber, episodeNumber, episodeTitle, isLatest } = req.body;
        ...
        const data = {
            ...
            director: director || '',
            cast: cast || '',
            translator: translator || 'Not translated',
...
app.delete('/api/admin/contents/:id', authMiddleware, subAdminOrAbove, async (req, res) => {
...
app.put('/api/admin/subscriptions/:id', authMiddleware, headAdminMiddleware, async (req, res) => {
    ...
    txn.processedBy = req.user.email;
    ...
    if (status === 'approved') {
        ...
        const exp = new Date();
        exp.setDate(exp.getDate() + (days[txn.duration] || 30));
        await User.findByIdAndUpdate(txn.userId, {
            subscription: { plan: txn.plan, duration: txn.duration, expiresAt: exp, startDate: new Date(), status: 'active', maxDevices: 6, approvedBy: req.user.email, approvedAt: new Date() }
        });
        if (user) {
            user.notifications.push({ message: 'Your ' + txn.plan.toUpperCase() + ' subscription has been APPROVED!', type: 'success' });
            await user.save();
        }
        ...
        if (user) {
            user.notifications.push({ message: 'Your subscription has been archived.', type: 'system' });
            await user.save();
        }
...
app.get('/api/admin/users', authMiddleware, headAdminMiddleware, async (req, res) => {
    res.json(await User.find({ role: { $ne: 'admin' } }).select('-password').sort({ createdAt: -1 }));
});

app.get('/api/admin/users/:id', authMiddleware, headAdminMiddleware, async (req, res) => {
...
app.delete('/api/admin/users/:id', authMiddleware, headAdminMiddleware, async (req, res) => {
...
app.put('/api/admin/flagged-users/:id', authMiddleware, headAdminMiddleware, async (req, res) => {
...
app.get('/api/admin/comments', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    contents.forEach(c => {
        c.comments.forEach(com => {
            all.push({ contentTitle: c.title, ...com.toObject() });
        });
    });
    res.json(all);
});

app.delete('/api/admin/comments/:contentId/:commentId', authMiddleware, headAdminMiddleware, async (req, res) => {
    const c = await Content.findById(req.params.contentId);
    if (!c) return res.status(404).json({ error: 'Not found' });
    c.comments.id(req.params.commentId).deleteOne();
    await c.save();
    res.json({ success: true });
});
...
app.get('/api/ads', async (req, res) => {
    res.json(await Ad.find({ isActive: true }).sort({ createdAt: -1 }));
});
...
app.post('/api/admin/ads', authMiddleware, subAdminOrAbove, upload.single('adMedia'), async (req, res) => {
    ...
    const ad = await Ad.create({
        ...
        targetPlans: targetPlans ? targetPlans.split(',') : [],
        createdBy: req.user.email
    });
    res.json({ success: true, ad });
});

app.put('/api/admin/ads/:id', authMiddleware, subAdminOrAbove, async (req, res) => {
    await Ad.findByIdAndUpdate(req.params.id, { ...req.body, updatedAt: new Date() });
    res.json({ success: true });
});

app.delete('/api/admin/ads/:id', authMiddleware, subAdminOrAbove, async (req, res) => {
    await Ad.findByIdAndDelete(req.params.id);
    res.json({ success: true });
});

// ========== PAYMENTS (HEAD ADMIN ONLY) ==========
app.get('/api/admin/payments', authMiddleware, headAdminMiddleware, async (req, res) => {
    const transactions = await Transaction.find({ status: 'approved' }).sort({ createdAt: -1 });
    const totalRevenue = transactions.reduce((sum, t) => sum + t.amount, 0);
    res.json({ transactions, totalRevenue });
});

app.post('/api/admin/withdraw', authMiddleware, headAdminMiddleware, async (req, res) => {
...
app.get('/api/admin/withdrawals', authMiddleware, headAdminMiddleware, async (req, res) => {
...
app.put('/api/admin/withdrawals/:id', authMiddleware, headAdminMiddleware, async (req, res) => {
    await Withdrawal.findByIdAndUpdate(req.params.id, { status: req.body.status, completedAt: new Date() });
    res.json({ success: true });
});
...
app.get('/api/admin/stats', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    const totalRevenue = (await Transaction.aggregate([{ $match: { status: 'approved' } }, { $group: { _id: null, total: { $sum: '$amount' } } }]))[0]?.total || 0;
    const totalTransactions = await Transaction.countDocuments({ status: 'approved' });
    res.json({ totalContent, totalUsers, totalRevenue, totalTransactions });
});
...
createAdmins().then(() => {
    ...
    app.listen(PORT, '0.0.0.0', () => {
        console.log('\n╔════════════════════════════════════════╗');
        console.log('║  AGASOBANUYE MOVIES | AGNEWS          ║');
        console.log('║  Port: ' + PORT + '                            ║');
        console.log('║  Admin: agasobanuyenews@gmail.com   ║');
        console.log('║  Version: v7 - Fixed & Secured      ║');
        console.log('╚════════════════════════════════════════╝\n');
    });
});
